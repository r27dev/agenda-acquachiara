...
export function CalendarGrid({ 
  ...
  for (let i = 0; i < firstDay; i++) {
    days.push(
      <div 
        key={`empty-${i}`} 
        className="min-h-[120px] border-r border-b border-border bg-background/50"
      />
    )
  }
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    days.push(
      <div
        key={day}
        className={cn(
          "min-h-[120px] border-r border-b border-border p-2 transition-colors hover:bg-secondary/50 cursor-pointer group relative",
